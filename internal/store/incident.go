...
const cols = `id,dedup_key,channel,channel_id,source,kind,severity,status,version,title,body,host,link,labels,
...
func (s *Store) Insert(in *Incident) error {
	...
	res, err := s.db.Exec(`insert into incidents
        (dedup_key,channel,channel_id,source,kind,severity,status,version,title,body,host,link,labels,
         desired_present,content_hash,message_id,stale_message_id,created_at,last_seen_firing,confirmed,heartbeat)
        values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
		in.DedupKey, in.Channel, in.ChannelID, in.Source, in.Kind, in.Severity, in.Status, in.Version,
...
func scan(rows *sql.Rows) (*Incident, error) {
	...
	if err := rows.Scan(&in.ID, &in.DedupKey, &in.Channel, &in.ChannelID, &in.Source, &in.Kind, &in.Severity, &in.Status, &in.Version,

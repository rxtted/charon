...
type Adapter interface {
    Name() string
    // Match inspects the request and returns the parsed Event, or ErrNotMatched
    // if this adapter does not claim it.
    Match(r *http.Request) (Event, error)
}
```

adapters self-register into a package-level registry at init. ingest buffers the request body (so each adapter can read it), then tries registered adapters in order; the first that returns without `ErrNotMatched` wins and its `Event` goes to the core. no adapter claiming the request is a 4xx and a log line, not a silent drop: a controlled emitter posting a shape nothing recognises is a bug worth surfacing.
...
- **firing, active incident exists**: update the fields from the new event and edit the message in place (a severity change flips the accent amber to red on the existing message). never a second message. then apply the re-notification decision below.
...
re-notification: charon owns it. a background sweep re-pings an incident that is active, not acknowledged, not snoozed, and last notified longer ago than the configured interval (default a few hours). because the board holds one message per incident and there are no threads to orphan, a re-ping is a delete-and-repost: the stale message is removed and a fresh one posted at the bottom of the channel, which resurfaces the incident with a new notification and keeps the one-message-per-incident invariant. a snooze expiring is the same repost, driven by the timer instead of the interval.
...
- **acknowledge / resolve**: a discord interaction must be answered within 3 seconds. the store write is sub-millisecond SQLite, so the normal path answers directly with an update-message (acknowledge edits the card, resolve deletes it). the deferred-update response is available as a safety valve if a handler ever needs longer, but v1 does not expect to need it.
...
discord rate-limits message operations per channel (message creation buckets at roughly five per five seconds) and globally (about fifty requests a second), and repeated 429s risk a day-long ip ban. the disgo rest client is bucket-aware and waits on a 429 itself, but charon adds a per-channel serialized send queue on top so that create, edit, and delete for a given incident stay ordered, and a burst of alerts into one channel drains in order rather than racing. an alert storm of a couple dozen conditions is well under the global ceiling and clears through the queue in a few seconds.
...
- `dedup_key` (unique among active rows), `channel`, `severity`, `status`
- `title`, `body`, `host`, `link`, `labels` (json)
- `message_id`, `channel_id` (the discord ids for edit and delete)
- `created_at`, `acked_at`, `acked_by`, `snoozed_until`, `last_notified_at`, `resolved_at`

active-incident lookup is by `dedup_key` where `resolved_at is null`. the snooze and re-notification sweeps index `snoozed_until` and `last_notified_at`. closed incidents keep their rows for MTTR and for answering "did it resolve, or did charon die", which a deleted board message alone cannot answer.
...
- **discord unreachable / gateway down**: ingest keeps accepting and recording incidents; sends queue and retry with backoff; disgo reconnects the gateway on its own. buttons are dead during the outage, but no incident state is lost, and on reconnect charon reconciles (below). a core-to-notification failure is visible in charon's own logs and its `/metrics`, not swallowed.
- **charon restart**: all state is in SQLite. on boot charon reconciles active incidents against discord: an active incident whose message still exists keeps its id; one whose message was deleted out of band is reposted; a closed incident is left closed. this is why the message id is persisted, not held in memory.
...
- send-queue ordering and 429 backoff against a fake discord client.
...
- a charon restart with active incidents reposts nothing already present and reattaches the existing message ids.
- prometheus scrapes charon's `/metrics`; stopping charon raises the charon-down alert (outside the triton SPOF).
- an unrecognised ingest payload returns 4xx and logs; an unknown tag routes to the fallback channel.
...
- the `Event` contract and the `Adapter` interface as the two boundaries the core depends on.
- the invariants: one active incident per `DedupKey`; the channel shows only active incidents (delete on resolve, not edit-to-resolved); charon owns re-notification, not grafana.
